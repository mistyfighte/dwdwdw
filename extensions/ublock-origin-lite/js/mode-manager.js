...
export async function readFilteringModeDetails(bypassCache = false) {
    ...
    // Bundled in YouTube Glass: upgrade legacy optimal-all-urls installs to
    // complete so generic cosmetic filters + scriptlets run everywhere.
    let bootstrapModified = false;
    if ( userModes.optimal.has('all-urls') && userModes.complete.has('all-urls') === false ) {
        userModes.optimal.delete('all-urls');
        userModes.complete.add('all-urls');
        bootstrapModified = true;
    }
    // Keep youtube.com on optimal: complete-mode generic cosmetic filtering
    // breaks Return YouTube Dislike's injected counter, and YouTube-specific
    // (optimal) filters already cover ad blocking there.
    if ( userModes.complete.has('all-urls') &&
        lookupFilteringMode(userModes, 'youtube.com') === MODE_COMPLETE
    ) {
        applyFilteringMode(userModes, 'youtube.com', MODE_OPTIMAL);
        bootstrapModified = true;
    }
    if ( bootstrapModified ) {
